...
* :doc:`02-split`
  ...
  - 변환된 텍스트들의 크기가 너무 작거나 크면, 지식을 찾을 때 정확도가 떨어질 수 있습니다. 적절한 크기로 쪼개어 조정한 문서(Document)로 변환합니다.
...
* :doc:`03-embed`
  ...
  - 검색에서는 질문과 유사한 문서를 찾습니다. 유사한 문서를 찾는 방법으로서 벡터 데이터를 활용하는 데요. 각 문서들을 미리 벡터 데이터로 변환해두면, 검색 시에는 질문과 저장된 지식 문서 간의 유사도만 비교하면 됩니다.
...
* :doc:`04-store`
  ...
  - 유사도 데이터를 디스크에 저장하여, 계속 사용할 수 있도록 준비
  - 문서 내용이 바뀌면, 벡터 데이터도 매번 재생성해야 합니다.
...
* :doc:`05-search`
  ...
  - `<../glance>`_\ 에서 소개한 코드를 참고해서 작성해보세요.
