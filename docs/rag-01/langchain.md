...
```{admonition} 랭체인 버전은 자세한 설명은 다음 튜토리얼에서 진행하겠습니다.
:class: attention
...
VectorStore는 [FAISS](https://python.langchain.com/docs/integrations/vectorstores/faiss/)를 사용합니다.

```{code-block} bash
:caption: 의존 라이브러리 설치

pip install -U langchain-community langchain-openai langchain-text-splitters faiss-cpu tiktoken
```
...
LangChain/LangGraph 버전도 기대해주세요. 🥳
